...
from vpscenedetect.platform import STRING_TYPE
import vpscenedetect.frame_timecode
...
def validate_capture_framerate(video_names, cap_framerates, framerate=None):
    ...
    if framerate is not None:
        if isinstance(framerate, float):
            if framerate < vpscenedetect.frame_timecode.MINIMUM_FRAMES_PER_SECOND_FLOAT:
        ...
        unavailable_framerates = [(video_names[i][0], video_names[i][1]) for
                                  ...
                                  vpscenedetect.frame_timecode.MINIMUM_FRAMES_PER_SECOND_FLOAT]
...
def validate_capture_parameters(video_names, cap_frame_sizes, check_framerate=False,
    ...
    """
    bad_params = []
    max_framerate_delta = vpscenedetect.frame_timecode.MINIMUM_FRAMES_PER_SECOND_FLOAT
